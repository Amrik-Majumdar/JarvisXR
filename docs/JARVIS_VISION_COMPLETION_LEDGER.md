...
| Accessible output | Priority speech, session cancellation, reading controls, directional and status haptics, visible text, VoiceOver labels, Dynamic Type, contrast, reduced motion, and a local device acceptance report | Speech and haptic policy tests, UI tests, and 29 screenshot proof states | VoiceOver focus and rotor, maximum text sizes, actual sound routes, interruption recovery, and haptic feel |
...
- Visual proof count: expected 29, run result pending
