...
| Detector | Model-agnostic interface plus checksum-pinned Apple `YOLOv3TinyInt8LUT`, 80-class catalog, schema validation, manifest, and notice | Model fetch and audit, compiled bundle check, real `Desk_chair.jpg` simulator inference, independent native-observation evaluation | Accuracy at practical distances and lighting, latency, memory pressure, and supported-target behavior |
